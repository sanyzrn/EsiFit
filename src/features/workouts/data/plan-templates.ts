...
export async function createPlanFromTemplate(userId: string, templateKey: PlanTemplateKey) {
  ...
  // One repeating week (week 1); plan days are identified by week+day.
  for (let dayIdx = 0; dayIdx < tpl.days.length; dayIdx++) {
    const spec = tpl.days[dayIdx];
    const planDay = await db.workoutPlanDay.create({
      data: {
        planId: plan.id,
        weekNumber: 1,
        dayNumber: dayIdx + 1,
...
export async function getTodayPlanDay(userId: string) {
  ...
  // Rotate through training days by day-of-year; deterministic per user.
  const seed = userId.split("").reduce((acc, c) => acc + c.charCodeAt(0), 0);
  const dayOfYear = Math.floor(
    (Date.now() - new Date(new Date().getFullYear(), 0, 0).getTime()) / 86_400_000,
  );
  const offset = (dayOfYear + seed) % trainingDays.length;
